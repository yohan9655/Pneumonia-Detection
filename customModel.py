...
def layers(initial):
    ...
    layer2 = tf.keras.layers.Conv2D(128,kernel_size = kernel, activation = 'relu', padding='same')(maxPool1)
    layer3a = tf.keras.layers.Conv2D(128,kernel_size = kernel, activation = 'relu', padding='same', kernel_regularizer=tf.keras.regularizers.l1(0.01), activity_regularizer=tf.keras.regularizers.l2(0.01))(layer2)
    layer3a= BatchNormalization()(layer3a)
    layer3b = tf.keras.layers.Conv2D(128,kernel_size = (4,4), activation = 'relu', padding='same')(layer2)
    layer4 = tf.keras.layers.concatenate([layer3a, layer3b], axis = 3)
    maxPool2 = tf.keras.layers.MaxPooling2D(strides=2)(layer4)
    layer5 = tf.keras.layers.Conv2D(256,kernel_size = kernel, activation = 'relu', padding='same')(maxPool2)
    layer6 = BatchNormalization()(layer5)
    layer7 = tf.keras.layers.Conv2D(256, kernel_size = kernel,activation = 'relu', padding='same', kernel_regularizer=tf.keras.regularizers.l1(0.01), activity_regularizer=tf.keras.regularizers.l2(0.01))(layer6)
    layer10 = tf.keras.layers.Conv2D(256, kernel_size = kernel,activation = 'relu', padding='same')(layer7)
    return layer10
...
def CustomNet():  
    ...
    layer3 = tf.keras.layers.Conv2D(64, kernel_size = kernel,activation = 'relu', padding='same')(layer1)
    layer4a = layers(layer2)
    layer4b = layers(layer3)
    layer5 = tf.keras.layers.concatenate([layer4a,layer4b], axis = 3)
    maxPool1 = tf.keras.layers.MaxPooling2D(strides=2)(layer5)
    layer6 = tf.keras.layers.Conv2D(256, kernel_size = kernel,activation = 'relu', padding='same')(maxPool1)
    layer7 = tf.keras.layers.Conv2D(128, (3, 3), activation='relu', dilation_rate = 2, padding = 'same')(layer6)
    layer8 = tf.keras.layers.Conv2D(64, (3, 3), activation='relu', dilation_rate = 2, padding = 'same')(layer7)
    final = tf.keras.layers.Conv2D(64, (3, 3), activation='relu', dilation_rate = 1, padding = 'same')(layer8)
